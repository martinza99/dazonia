...
<body>
	...
	echo '<table>';
	if ($user->isAdmin)
		echo '<tr><th><a href="/login/token.php" style="color:#2196F3;"><button>#</a></th><th>Name</th><th>LastLogin</th><th>ResetPW</th><th><button>X</button></th></tr>';
	else
		echo '<tr><th>#</th><th>Name</th></tr>';
	while ($rows = $result->fetch_object()) {
		echo "<tr id=\"$rows->id\">";
		echo "<td>$rows->id</td>";
		echo "<td><a href=\"/list/?q=u%3A$rows->id\" target=\"_top\">$rows->name</a>"; //print name
		if ($user->isAdmin) {
			$timeStamp = timeElapsed($rows->lastLogin);
			echo "<td title=\"$rows->lastLogin\">$timeStamp</td>"; //print last login
			echo "<td><a href=\"/login/resetPassword.php?resetKey=$rows->apiKey\" target=\"_top\">Link</a>"; //print password reset link 
