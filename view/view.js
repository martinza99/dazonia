	...
	if (_xhr != "none") {
		...
		if (_push) {
			history.pushState(
				stateObj,
				_xhr,
				location.origin + "/view/" + _xhr
			);
			document.querySelector(".hiddenVal").value =
				location.origin + "/files/" + _xhr;
		}
	}
}

function keyDown(_event) {
	if (document.activeElement.classList.contains("disableHotkeys")) return;
	let prevent = true;
	if (!_event.altKey) {
		switch (_event.key) {
			case "c":
		...
		if (tag != undefined) sendTagValue(tag);
		else prevent = false;
	}
	if (prevent) {
...
function openStars() {
	for (let index = 0; index <= 10; index++) makeStar(this, index);
}
		...
		function (response, status, xhr) {
			try {
				...
				if (response.success == true)
					$(star).attr(
						"src",
						"../list/img/" + response.avgrating + ".png"
					);
		...
		function (response, status, xhr) {
			try {
				...
				if (response.success == true) {
					...
					parent.insertBefore(
						container,
						parent.childNodes[parent.childElementCount - 2]
					);
				} else throw new Error(response.error);
			} catch (error) {
				alert(error);
...
function replaceChars() {
	//tag input keyup callback
	if (this.value.toLowerCase() != this.value)
		// if contains upperCase
		this.value = this.value.toLowerCase();
	while (this.value.includes(" ")) this.value = this.value.replace(" ", "_");
}

function stopSlide() {
	if (slide != undefined) clearInterval(slide);
}
